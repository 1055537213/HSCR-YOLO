...
class BaseTrainer:
    ...
    def validate(self):
        ...
        if metrics is None:
            return None, None
        fitness_metric = str(getattr(self.args, "fitness_metric", "") or "").lower().replace("_", "-")
        metric_key = {
        ...
        if metric_key:
            if metric_key in metrics:
                metrics["fitness"] = metrics[metric_key]
            else:
                LOGGER.warning(f"Requested fitness_metric={fitness_metric!r}, but {metric_key!r} was not available.")
