...
def _candidate_dataset_roots():
    roots = []
    env_root = os.getenv('DCAS_DATASET_ROOT')
    if env_root:
        roots.append(Path(env_root))

    yaml_path = Path(__file__).resolve().parents[1] / 'datasets' / 'VisDrone.yaml'
    if yaml_path.exists():
        try:
            ...
            if yaml_root:
                roots.append(Path(yaml_root))
        except Exception:
            pass

    roots.append(Path('/home/datasets/VisDrone'))
    return roots
...
def resolve_dataset_root():
    for root in _candidate_dataset_roots():
        ...
        if root.exists():
            return str(root)
    raise FileNotFoundError('Could not resolve the DCAS dataset root. Set DCAS_DATASET_ROOT or update VisDrone.yaml.')
...
def _dam_cache_path(dataset_root):
    return Path(dataset_root) / '.dam_cache.json'


def _dam_cache_signature(dataset_root, img_size):
    label_root = Path(dataset_root) / 'labels' / 'train'
    label_files = list(label_root.glob('*.txt')) if label_root.exists() else []
    latest_mtime = max((f.stat().st_mtime for f in label_files), default=0.0)
    return {
        'dataset_root': str(Path(dataset_root).resolve()),
        'img_size': int(img_size),
        'num_label_files': len(label_files),
        'latest_label_mtime': latest_mtime,
    }


def _load_dam_cache(dataset_root, img_size):
    cache_path = _dam_cache_path(dataset_root)
    if not cache_path.exists():
        return None
    try:
        with open(cache_path, 'r', encoding='utf-8') as f:
            payload = json.load(f)
        if payload.get('signature') == _dam_cache_signature(dataset_root, img_size):
...
def _save_dam_cache(dataset_root, img_size, stats):
    cache_path = _dam_cache_path(dataset_root)
    payload = {
        'signature': _dam_cache_signature(dataset_root, img_size),
        'stats': stats,
    }
...
def label_counts(img_size=640, dataset_root=None):
    dataset_root = dataset_root or resolve_dataset_root()
    cached = _load_dam_cache(dataset_root, img_size)
    if cached:
        ...
        return cached['small_ratio']

    dataset = YOLODataSet(yolo_root=dataset_root)
    ...
    if small_objects_count:
        print(
            f'WARNING: Extremely small objects found. {small_objects_count} of {len(wh0)} labels are < 1 pixels in size.')
    ...
    results = [(i * 8, (i + 1) * 8, int(count), float(count / total_anchors)) for i, count in enumerate(label_counts)
               if count >= 0]
    ...
    _save_dam_cache(dataset_root, img_size, {'small_ratio': float(results[0][3]), 'results': results})
    return results[0][3]


def num_DAM(dataset_root=None):
    def function(J):
        R = 0.15
        k = 30
        return math.floor(k * (J - R)) + 3

    J = label_counts(dataset_root=dataset_root)
    N = function(J)
